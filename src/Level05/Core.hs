...
runApp = do
  ...
  case cfgE of
    Left err   -> do
      print err -- We can't run our app at all! Display the message and exit the application.
    Right cfg ->
...
prepareAppReqs = let 
           appdb = Conf.firstAppConfig 
           in do
           x <- DB.initDB (Conf.dbFilePath appdb)
...
mkAddRequest ti c = AddRq <$> mkTopic ti <*> (mkCommentText . decodeUtf8 . LBS.toStrict) c
